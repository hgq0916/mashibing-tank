...
public class LightTank extends AbstractTank{
  ...
  public LightTank(int x, int y, Dir dir,TankFrame tf,Group group) {
  ...
  private void boundaryDetect() {
    if(x<2) this.x = 2;
    if(y<32) this.y = 32;
    if(x+ LightTank.TANK_WIDTH>TankFrame.WIN_WIDTH-2) x = TankFrame.WIN_WIDTH- LightTank.TANK_WIDTH-2;
    if(y+ LightTank.TANK_HEIGHT>TankFrame.WIN_HEIGHT-2) y = TankFrame.WIN_HEIGHT- LightTank.TANK_HEIGHT-2;
  }
